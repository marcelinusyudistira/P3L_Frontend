<template>
    <v-main class="list">    
        <v-container class="grey lighten-5" style="margin-top: 10px">
            <v-toolbar class="mb-2" color="indigo darken-5" dark flat>
                <v-toolbar-title>Jadwal Customer Service</v-toolbar-title>
                <v-text-field
                    class="btnSearch2"
                    v-model="search"
                    label="Search"
                ...
                <v-btn class="ml-4" color="success" dark @click="dialogCs = true"> Tambah </v-btn>
            </v-toolbar>
            <v-card elevation="5">
                <v-row no-gutters>
                <v-col cols="12" sm="4">
                    <v-simple-table class="headerTable">
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsSelasa" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                ...
                <v-col cols="12" sm="4">
                    <v-simple-table>
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsRabu" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                                        <v-icon>mdi-pencil</v-icon>
                                    </v-btn>
                ...
                <v-col cols="12" sm="4">
                    <v-simple-table>
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsKamis" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                                        <v-icon>mdi-pencil</v-icon>
                                    </v-btn>
                ...
                <v-col cols="12" sm="4">
                    <v-simple-table class="mt-5">
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsJumat" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                                        <v-icon>mdi-pencil</v-icon>
                                    </v-btn>
                ...
                <v-col cols="12" sm="4">
                    <v-simple-table class="mt-5">
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsSabtu" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                                        <v-icon>mdi-pencil</v-icon>
                                    </v-btn>
                ...
                <v-col cols="12" sm="4">
                    <v-simple-table class="mt-5">
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in pegawaisCsMinggu" :key="item.nama_pegawai">
                                    <td class="text-left">Shift {{item.shift}} - {{ item.nama_pegawai }}</td>
                                    <v-btn class="ml-1" outlined fab color="teal" x-small @click="editHandlerCs(item)">
                                        <v-icon>mdi-pencil</v-icon>
                                    </v-btn>
        ...
        <v-dialog v-model="dialogCs" persistent max-width="600px">
            <v-card>
                <v-card-title>
...
<script>
  export default {
    ...
    methods: {
        ...
        readDataHari(hari) {
            var url = this.$api + '/indexHari/3/' + hari;
            this.$http.get(url, {
                headers: {
                    'Authorization' : 'Bearer ' + localStorage.getItem('token')
                }
                ...
                if(hari == "selasa")
                    this.pegawaisCsSelasa = response.data.data;
                else if(hari == "rabu")
                    this.pegawaisCsRabu = response.data.data;
                else if(hari == "kamis")
                    this.pegawaisCsKamis = response.data.data;
                else if(hari == "jumat")
                    this.pegawaisCsJumat = response.data.data;
                else if(hari == "sabtu")
                    this.pegawaisCsSabtu = response.data.data;
                else if(hari == "minggu")
                    this.pegawaisCsMinggu = response.data.data;
            })
        },
        ...
        readDataSearch() {
            var url = this.$api + '/searchJadwal/3/' + this.search;
            this.$http.get(url, {
                headers: {
                    'Authorization' : 'Bearer ' + localStorage.getItem('token')
                }
